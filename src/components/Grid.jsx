...
export function Grid({ placements, hoveredCell }) {
  const { CELL_SIZE, CONTAINER_WIDTH, HAND_ROW_TOP, HAND_SLOT_SIZE } = useSizes()
  ...
  return (
    <>
      ...
      <div style={{
        ...
        <div className="grid-scanline" style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 0 }} />
        {Array.from({ length: GRID_ROWS }, (_, row) =>
          Array.from({ length: GRID_COLS }, (_, col) => {
            const occupantId = Object.entries(placements).find(([, pos]) => pos?.row === row && pos?.col === col)?.[0]
            const occupant = occupantId ? getEntity(occupantId) : null
            const isHovered = hoveredCell?.row === row && hoveredCell?.col === col
            return (
              <GridCell key={`${row}-${col}`} row={row} col={col}
                occupant={occupant} isHovered={isHovered} cellSize={CELL_SIZE} />
            )
          })
        )}
      </div>
      ...
      {/* ── hand-dock separator ─────────────────────────────── */}
      <div style={{
        position: 'absolute', top: HAND_ROW_TOP - 12, left: 0, right: 0,
      ...
      {/* ── hand slot outlines ───────────────────────────────── */}
      <div style={{ position: 'absolute', top: HAND_ROW_TOP, left: 0, display: 'flex', width: CONTAINER_WIDTH }}>
        {ENTITIES.map((entity) => {
          const isPlaced = placements[entity.id] !== null
          return (
            <div key={entity.id} style={{
              width: HAND_SLOT_SIZE, height: CELL_SIZE,
              border: `1px dashed ${entity.colors.stroke}${isPlaced ? '35' : '20'}`,
              borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center',
              transition: 'border-color 0.25s',
            }}>
              {isPlaced && (
                <span style={{ fontSize: 10, fontFamily: "'JetBrains Mono', monospace", color: entity.colors.dimText, letterSpacing: '0.1em' }}>
                  {entity.abbr}
                </span>
              )}
            </div>
...
function GridCell({ row, col, occupant, isHovered, cellSize }) {
    ...
    <div style={{
      ...
      {!occupant && (
        <span style={{
          ...
          fontSize: Math.max(7, cellSize * 0.09),
          fontFamily: "'JetBrains Mono', monospace",
          color: 'rgba(148,163,184,0.1)', userSelect: 'none', pointerEvents: 'none',
        }}>
          {['A','B','C','D','E'][col]}{row + 1}
        </span>
      )}
      <AnimatePresence>
        {isHovered && (
          <motion.div key="glow" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            style={{ position: 'absolute', inset: 0, background: 'radial-gradient(circle at center, rgba(99,102,241,0.18) 0%, transparent 70%)', pointerEvents: 'none' }}
          />
        )}
      </AnimatePresence>
    </div>
