...
export function useGameState() {
  ...
  // Live results – used internally for allPlaced check only; NOT shown in sidebar
  const liveResults = useMemo(() => evaluatePuzzle(puzzle, placements), [puzzle, placements])
  ...
  /** COMPILE — submit the current board state. */
  const compile = useCallback(() => {
    ...
    if (correct) { setGameStatus('won');  return 'won'  }
  ...
  return {
    puzzle, dayNumber,
    placements,
    /** Results from the last COMPILE (null = never compiled). Sidebar shows these. */
    lastCompileResults,
    attempts, gameStatus,
    allPlaced,
